<template>
    <div>
        <div>
            <b-table :items="itemsCalendar" :fields="fieldsTable" ref="table" bordered hover fixed>
                <template v-slot:cell()="data">
                    <div  @click="showModal(data.value)">
                    <b>{{viewData(data.value).date}}</b>
                    <p>_______________</p>
                    <p v-for="client in viewData(data.value).client">{{client}}</p>
                    </div>
                </template>
            </b-table>
        </div>
        <div>
            <b-modal id="modal" ref="modal" centered ok-only>
                <b-table :items="itemsDayList" dark bordered hover fixed no-body>
                    <template v-slot:cell(Client)="data">
                        <b-card v-if="data.value" overlay img-src="https://picsum.photos/900/250/?image=3">
                            {{data.value}}
                        </b-card>
                    </template>
                </b-table>
            ...
            <iframe src="https://calendar.google.com/calendar/embed?src=3heg1bu3gvoqmplmp5kpqn56ic%40group.calendar.google.com&ctz=America%2FLos_Angeles"
            ...
            getItemsCalendar() {
                ...
                while ((count - (6 - this.get.getDay())) < 22) {
                    ...
                    if (date === 7) {
                        date = 0
                        item['_cellVariants'] = {
                            date0: 'danger', date6: 'danger', date1: 'info', date2: 'info',
